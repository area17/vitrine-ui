...
eslint() {
    ...
    if [ "$FILES" = "." ]; then
        if ! $ESLINT --no-config-lookup  -c .eslintrc.js --fix .; then
            fatalError "Eslint finished with errors"
        fi
    else
        for FILE in $FILES; do
            if ! $ESLINT --no-config-lookup  -c .eslintrc.js --fix  "$FILE"; then
                fatalError "Eslint finished with errors"
            fi
        done
    fi
...
stylelint() {
    ...
    if [ "$FILES" = "." ]; then
        if ! $STYLELINT "resources/**/*.{css,scss,sass}" "--aei" "--fix" "--cache" ; then
            fatalError "Stylelint finished with errors"
        fi
    else
        for FILE in $FILES; do
            if ! $STYLELINT "$FILE" "--aei" "--fix" "--cache"; then
                fatalError "Stylelint finished with errors"
            fi
        done
    fi
...
prettier() {
    ...
    if [ "$FILES" = "." ]; then
        if ! $PRETTIER --log-level=error --quiet --write .; then
            fatalError "Prettier finished with errors"
        fi
    else
        for FILE in $FILES; do
            if ! $PRETTIER --log-level=error --quiet --write "$FILE"; then
                fatalError "Prettier finished with errors"
            fi
        done
    fi
