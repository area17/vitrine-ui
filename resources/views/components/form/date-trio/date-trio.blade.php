...
<fieldset data-behavior="DateTrio"
          ...
          {{ $attributes->class(Arr::toCssClasses(['s-disabled' => $disabled, 's-error' => $error, 's-readonly' => $readonly])) }}
          {{ $dataAttrs }}
          {{ $disabled ? 'disabled' : '' }}
          {{ $disabled ? 'inert' : '' }}>
    ...
    <ol class="mt-20 flex flex-row flex-wrap gap-gutter sm:flex-nowrap">
        <li class="w-full sm:w-auto sm:flex-1">
            <div data-behavior=""
                 {{ $attributes->class(Arr::toCssClasses([$ui('input', 'base'), 's-disabled' => $disabled, 's-error' => $error, 's-readonly' => $readonly])) }}
                 {{ $disabled ? 'inert' : '' }}>
