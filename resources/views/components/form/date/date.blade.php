...
<div @if ($picker) data-DatePicker-el="{{ $pickerID }}" @endif
     {{ $attributes->merge(['data-behavior' => $attributes->prepends(implode(' ', $behaviors))])->class(Arr::toCssClasses([$ui('input', 'base'), 's-disabled' => $disabled, 's-error' => $error, 's-readonly' => $readonly])) }}
     {{ $dataAttrs }}
     {{ $disabled ? 'inert' : '' }}>
